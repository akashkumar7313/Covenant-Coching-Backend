...
const OurDirectorsImages = () => {
  ...
  const handleImageDelete = (imageId) => {
    const imageRef = firebase.database().ref(`Director/${imageId}`);
  ...
  const handleSaveButtonClick = () => {
    if (selectedImage) {
      setUploading(true);
      const storageRef = firebase.storage().ref(`Director/${selectedImage.name}`);
      ...
      uploadTask.on(
        ...
        () => {
          uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
            firebase.database().ref("Director").push({
              url: downloadURL,
...
export default OurDirectorsImages;
